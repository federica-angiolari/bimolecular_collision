...
def main(mod, n_angol, program, scr_dir, num_t, num_p, n, path_program):
    ...
    if program == 'GAMESS':
        for i in range(n):
            print("Calcolo dinamica: {}".format(i))
            os.system("{} geometry_{}.inp > log".format(path_program, i))
    ...
    if mod == 'coll':
        analisi_collisione.main(n, num_atom, num_p, program, n_angol)
    else:
        analisi_sm.main(num_t, n)
